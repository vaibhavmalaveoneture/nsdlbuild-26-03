...
BEGIN
    SET NOCOUNT ON;
   if exists (select 1 from nsdlcaf.dbo.fvci_applications WHERE application_id = @FvciApplicationId)
   begin
    IF @Status = '0'
    BEGIN
        -- Only UPDATE existing record when status = '0'
        UPDATE nsdlcaf.dbo.fvci_kyc_documents
        SET 
            status = @Status,
    ...
    ELSE
    BEGIN
    MERGE INTO nsdlcaf.dbo.fvci_kyc_documents AS target
