...
BEGIN
    ...
    FROM nsdlcaf.dbo.draft_TaxResidency WHERE ApplicationId = @ApplicationId;
   	...
   	INSERT INTO nsdlcaf.dbo.RegistrationForm (
	...
	SELECT 
	    ...
	    DdpRegistrationNumber, CustodianRegistrationNumber, DpRegistrationNumber,
	    SelectedCity, HasOtherEntity, OtherEntityDetails, CreatedAt, UpdatedAt
	FROM nsdlcaf.dbo.draft_RegistrationForm
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.CustodianInfo (
	...
	FROM nsdlcaf.dbo.draft_CustodianInfo
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.DdpInfo (
	...
	FROM nsdlcaf.dbo.draft_DdpInfo
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.DesignatedBank (
	    ApplicationId, DesignatedBankAddress, CreatedAt, UpdatedAt
	)
	SELECT 
	    ApplicationId, DesignatedBankAddress, GETDATE(), GETDATE()
	FROM nsdlcaf.dbo.draft_DesignatedBank
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.DesignatedBankName (
	    ApplicationId, Name, Value, Address, CreatedAt, UpdatedAt
	)
	SELECT 
	    ApplicationId, Name, Value, Address, GETDATE(), GETDATE()
	FROM nsdlcaf.dbo.draft_DesignatedBankName
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.DisciplinaryHistory (
	...
	FROM nsdlcaf.dbo.draft_DisciplinaryHistory
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.DpInfo (
	    ApplicationId, Name, Value, RegistrationNumber, DdpId, CreatedAt, UpdatedAt
	)
	SELECT 
	    ApplicationId, Name, Value, RegistrationNumber, DdpId, GETDATE(), GETDATE()
	FROM nsdlcaf.dbo.draft_DpInfo
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.HasPan (
	    ApplicationId, HasPanRadio, HasPanNumber, CreatedAt, UpdatedAt
	)
	SELECT 
	    ApplicationId, HasPanRadio, HasPanNumber, GETDATE(), GETDATE()
	FROM nsdlcaf.dbo.draft_HasPan
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.PriorAssociation (
	    ApplicationId, PriorAssociationRadio, CreatedAt, UpdatedAt
	)
	SELECT 
	    ApplicationId, PriorAssociationRadio, GETDATE(), GETDATE()
	FROM nsdlcaf.dbo.draft_PriorAssociation
	WHERE ApplicationId = @ApplicationId;

	INSERT INTO nsdlcaf.dbo.PriorAssociationRow (
	...
	FROM nsdlcaf.dbo.draft_PriorAssociationRow
	WHERE ApplicationId = @ApplicationId;
	
	INSERT INTO nsdlcaf.dbo.ThroughGlobalCustodian (
	...
	FROM nsdlcaf.dbo.draft_ThroughGlobalCustodian
	WHERE ApplicationId = @ApplicationId;
	...
	insert into nsdlcaf.dbo.InvestmentManager(ApplicationId, NameOfEntity, TypeOfEntity, CountryCode, TelephoneNumber, FaxNumber, EmailId, CreatedAt, UpdatedAt)
	...
	SELECT 
	    ...
	    ConsentIntermediaryEmail2, ConsentIntermediaryEmail3, ConsentIntermediaryMobile, CreatedAt, UpdatedAt
	FROM 
	    nsdlcaf.dbo.draft_ConsentIntermediary  -- Assuming the draft table is named draft_ConsentIntermediary
	WHERE 
	    ApplicationId = @ApplicationId;
	...
	INSERT INTO nsdlcaf.dbo.InformationOfSaSmFvciApplicant (
	...
	SELECT 
	    ...
	    IdentityDocNumber, CreatedAt, UpdatedAt
	FROM 
	    nsdlcaf.dbo.draft_InformationOfSaSmFvciApplicant  -- Assuming the draft table is named draft_InformationOfSaSmFvciApplicant
	WHERE 
	    ApplicationId = @ApplicationId;
	-- Inserting data from draft_Signatory to the main Signatory table where ApplicationId matches
	INSERT INTO nsdlcaf.dbo.Signatory (
	    ApplicationId, Details, CreatedAt, UpdatedAt
	)
	SELECT 
	    ApplicationId, Details, CreatedAt, UpdatedAt
	FROM 
	    nsdlcaf.dbo.draft_Signatory  -- Assuming the draft table is named draft_Signatory
	WHERE 
	    ApplicationId = @ApplicationId;
	-- Inserting data from draft_OwnerDetails to the main OwnerDetails table where ApplicationId matches
	INSERT INTO nsdlcaf.dbo.OwnerDetails (
	...
	FROM 
	    nsdlcaf.dbo.draft_OwnerDetails o
	inner join nsdlcaf.dbo.Signatory s on o.SignatoryId = s.id  where  s.ApplicationId  = @ApplicationId;

	-- Inserting data from draft_MaterialShareholder to the main MaterialShareholder table where ApplicationId matches
	INSERT INTO nsdlcaf.dbo.MaterialShareholder (
	...
	SELECT 
	    ...
	    CreatedAt, UpdatedAt
	FROM 
	    nsdlcaf.dbo.draft_MaterialShareholder  -- Assuming the draft table is named draft_MaterialShareholder
	WHERE 
	    ApplicationId = @ApplicationId;
	-- Inserting data from draft_BeneficialOwners to the main BeneficialOwners table where ApplicationId matches
	INSERT INTO nsdlcaf.dbo.BeneficialOwners (
	...
	SELECT 
	    ...
	    PercentageStakeHeld, IndividualOrNonIndividual, CreatedAt, UpdatedAt
	FROM 
	    nsdlcaf.dbo.draft_BeneficialOwners  -- Assuming the draft table is named draft_BeneficialOwners
	WHERE 
	    ApplicationId = @ApplicationId;
	-- Inserting data from draft_Managers to the main Managers table where ApplicationId matches
	INSERT INTO nsdlcaf.dbo.Managers (
	...
	SELECT 
	    ...
	    CreatedAt, UpdatedAt
	FROM 
	    nsdlcaf.dbo.draft_Managers  -- Assuming the draft table is named draft_Managers
	WHERE 
	    ApplicationId = @ApplicationId;
