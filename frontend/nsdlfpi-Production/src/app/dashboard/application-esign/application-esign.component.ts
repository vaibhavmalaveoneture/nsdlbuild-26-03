...
import {FvciApplicationSaveService} from '../new-application/form-layout/fvci-application-save.service'
import {
  AuthService,
...
@Component({
  selector: 'app-application-esign',
  standalone: false,
  templateUrl: './application-esign.component.html',
  styleUrl: './application-esign.component.scss'
})
export class ApplicationEsignComponent {
 @ViewChild('dt2') dt2!: Table;
  ...
  fpiListEsign: FvciApplicationMaster[] = [];
  filteredFpiListEsign: FvciApplicationMaster[] = [];
  isVerify: any;
  selectedFPI: FPI | undefined;
  first = 0;
  searchFormEsign = new FormGroup({
  ...
  async ngOnInit() {
    ...
    this.searchFormEsign.reset({
      query: '',
      dateRange: null,
    });
  
    this.filteredFpiListEsign = [...this.fpiListEsign]; // Reset filter list
    if (this.dt2) {
      this.dt2.reset(); // Reset PrimeNG table filters
    }
  ...
  exportCSV() {
    ...
    this.filteredFpiListEsign.forEach((fpi, index) => {
  ...
  private initializeBreadcrumbs() {
    this.crumbItems = [
      { label: 'FVCI', route: '/new-application' },
      { label: 'List of FVCI', route: '/application-list' },
    ];
  }
  ...
  private async loadUserApplications() {
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.fvciApplicationSaveService.getFvciApplicationForATS(token)
      );
      if (response?.success) {
        this.fpiListEsign = this.filteredFpiListEsign = response.data;
      }
    } catch (err: any) {
      console.error(err);
    } finally {
  ...
  onSearch() {
    this.dt2.reset();
    const { dateRange, query } = this.searchFormEsign.value;

    // Start with a copy of the complete list.
    this.filteredFpiListEsign = [...this.fpiListEsign];
    ...
    if (dateRange && Array.isArray(dateRange) && dateRange[0] && dateRange[1]) {
      ...
      this.filteredFpiListEsign = this.filteredFpiListEsign.filter((fpi) => {
    ...
    if (query) {
      ...
      this.filteredFpiListEsign = this.filteredFpiListEsign.filter(
        (fpi) =>
  ...
  getSeverity(status: string): 'success' | 'info' | 'danger' | 'warn' | undefined {
    switch (status) {
      case 'Approved':
        return 'success';
      case 'Rejected':
