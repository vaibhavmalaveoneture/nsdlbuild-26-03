...
export class FvciFormComponent implements OnInit, AfterViewInit  {
  ...
  initialiseForm() {
    ...
    const romatchedCountry = this.countryCodes_pan.find(
    ...
    const fomatchedCountry = this.countryCodes_pan.find(
      (c) => c.code === focountryValue?.code
    );
    console.log("matchedCountry", focountryValue)
    this.formGroup.get('contactDetails.office.countryCode')?.setValue(focountryValue)
    ...
    const ciomatchedCountry = this.countryCodes_pan.find(
      (c) => c.code === ciocountryValue?.code
    );
    ...
    const iocountryValue = this.applicationData.data?.ekycForm?.contactDetails?.indianOffice.countryCode;
    const iomatchedCountry = this.countryCodes_pan.find(
      (c) => c.code === iocountryValue?.code
    );
    console.log("matchedCountry", iomatchedCountry)
    this.formGroup.get('contactDetails.indianOffice.countryCode')?.setValue(iomatchedCountry)
    this.formGroup.get('contactDetails.indianOffice.areaCode')?.setValue(this.applicationData.data?.ekycForm?.contactDetails?.indianOffice.areaCode)
    this.formGroup.get('contactDetails.indianOffice.number')?.setValue(this.applicationData.data?.ekycForm?.contactDetails?.indianOffice.number)
    ...
    rows?.forEach(row => {
      ...
      newRow.patchValue({
        details: {
          ...
          taxResidencyJurisdiction: this.countries.find(c => c.name === row.details?.taxResidencyJurisdiction),
          nationality: this.countries.find(c => c.name === row.details?.nationality)
        }
      });
  ...
  private initializeIncomeSource(incomeDetails: any) {
    ...
    if (incomeDetails.incomeSource?.length) {
      const sources = incomeDetails.incomeSource
        ...
        .filter(Boolean);
      incomeSourceControl?.setValue(sources);
    }
