import { Component, Input, OnInit, OnDestroy, ViewEncapsulation, ChangeDetectorRef,AfterViewInit , SimpleChanges, NgZone, input, ElementRef, ViewChildren, QueryList   } from '@angular/core';
...
export class FvciFormComponent implements OnInit, AfterViewInit  {
  ...
  private initializeManagingOfficials(rows: any[]) {
    if(rows.length==0 || this.countries_pan.length==0){
      return
    }
    ...
    rows?.forEach(row => {
      ...
      newRow.patchValue({
        details: {
          ...
          taxResidencyJurisdiction: row.details?.taxResidencyJurisdiction?.short_code
          ? this.countries_pan.find(c => c.short_code === row.details.taxResidencyJurisdiction.short_code)
          : null,
          nationality: row.details?.nationality?.short_code
          ? this.countries_pan.find(c => c.short_code === row.details.nationality.short_code)
          : null
        }
      });
  ...
  addRow(): void {
    ...
    row.get('trcNo')?.valueChanges.subscribe(() => {
      row.updateValueAndValidity({ onlySelf: true });
    });
  ...
  subscribe(): void{
    ...
    countryControl?.valueChanges.subscribe((selectedCountry: any) => {
      if (
        ...
        const matchingCountryCode = this.countries.find(
