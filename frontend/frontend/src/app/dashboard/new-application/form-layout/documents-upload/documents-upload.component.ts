  ...
  initializeData() {
    ...
    this.kycDocuments = this.applicationData.data.fvciKycDocument.map((doc: any) => {
      ...
      const controlName =
        ...
        documentType === 'POI' ? 'poiUpload' : null;
    ...
    this.additionalDocuments = (this.kycDocuments || [])
      ...
      console.log("this.documentsthis.documentsthis.documents",this.documents)
        ...
        try {
          ...
          if (response?.success) {
            ...
            if (doc) {
              ...
              const controlName = docType === 'POA' ? 'poaUpload' : docType === 'POI' ? 'poiUpload' : null;
              if (controlName) {
                const control = this.formGroup.get(controlName);
                if (control) {
                  control.setValidators([Validators.required]);
